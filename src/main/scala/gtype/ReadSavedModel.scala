...
import gtype.TypeAliasGraph.{TypeAliasing, typeAliasingsToGroundTruths, typeContextToAliasings}
...
object ReadSavedModel {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def evaluate(iterations: Int,
                 dataSet: (Map[Symbol, TypeAliasing],
                   List[(Symbol, Symbol)],
                   List[(Symbol, Symbol)])): Double = {
      val (symbolDefMap, posData, negData) = dataSet
      val (target, predictions) =
        forwardPredict(symbolDefMap, posData, negData, encodingBatch = 1, encodingIterations = iterations)
    ...
    for (testSize <- 100 to 800 by 100){
      val testSets = Vector.fill(6){generateTestData(JSExamples.realWorldExamples, testSize)}
