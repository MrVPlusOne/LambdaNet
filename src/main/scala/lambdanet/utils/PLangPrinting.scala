...
object PLangPrinting {
  ...
  def renderModuleToDirectory(
    ...
    val data = m.stmts
      .map {
        show(_, m.mapping, prediction, predSpace)
      }
    ...
    def rec(indent: Int, stmt: QStmt): Vector[(Int, Output)] = {
      ...
      stmt match {
        case VarDef(x, init, isConst) =>
          ...
          Vector(
//            indent -> span(keyword, " ", x.toString, showAnnot(x), " := ", init.toString)
            indent ->
        ...
        case WhileStmt(cond, body) =>
          (indent -> code("while (", cond, ")")) +: rec(indent, body)
        ...
        case FuncDef(funcName, args, ret, body) =>
          ...
          Vector(
            ...
            rec(indent, makeSureInBlockQ(body))
