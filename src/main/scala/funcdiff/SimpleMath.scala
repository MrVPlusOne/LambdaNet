...
//  }
    ...
    if (threadNum > 1) {
      ...
      class MasterClass(supervisor: ActorRef, system: ActorSystem) extends Actor {
        ...
        def receive: PartialFunction[Any, Unit] = {
          case StartTask =>
            (0 until math.min(threadNum, tasks.length)).foreach(i => slaves(i) ! getNextTask())
          case EvalResult(b) =>
            results = b :: results
            finished += 1
  ...
  def mutualInformation[X, Y](frequencies: Map[(X, Y), Int]): Double = {
    ...
    val pxy = frequencies.mapValues(_.toDouble / totalCount).view.force
    ...
    val (px, py) = {
      ...
      (
        countX.mapValues(_.toDouble / totalCount).view.force,
        countY.mapValues(_.toDouble / totalCount).view.force
      )
    }
